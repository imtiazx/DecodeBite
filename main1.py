...
# if LANGFLOW_TOKEN is None:
#     raise ValueError("LANGFLOW_TOKEN is not set. Check your .env file.")
...
st.set_page_config(page_title="DecodeBite", page_icon="🍎")
...
def generate_analysis(extracted_info):
    tweaks = {
        "TextInput-NDzDD": {"input_value": extracted_info},
    }
    ...
    # Accessing the text data from result
    report = result[0].outputs[0].results["text"].data["text"]
    return report
...
if selected_option == "Upload & Extract":
    ...
    uploaded_image = st.file_uploader("Upload an image of the ingredients label", type=["jpg", "png", "jpeg"])
    if uploaded_image:
...
if selected_option == "Analyze & Decode":
    ...
    st.markdown("<i><strong>Disclaimer:</strong> This analysis is generated by an LLM. Please review with subject matter experts.</i>", unsafe_allow_html=True)
